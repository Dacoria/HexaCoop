...
public class PlayerHealth : HexaEventCallback
{
    ...
    protected override void OnPlayerRocketHitTile(PlayerScript pWhoShot, Hex hex)
    {
        ...
        if (playerOnTile?.PlayerId == playerScript.PlayerId)
        {
            TakeDamage(1);
        }
    }
    ...
    {
        if (pHit.PlayerId == playerScript.PlayerId)
        {
            TakeDamage(1);
        }
    }
    ...
    {
        if (!player != playerScript && hexWithTargetHit.GetPlayerOnHex()?.PlayerId == playerScript.PlayerId)
        {
            TakeDamage(damage);
        }
    }
    ...
    {       
        if (hex.GetPlayerOnHex()?.PlayerId == playerScript.PlayerId)
        {
            TakeDamage(damage);
        }
    }
